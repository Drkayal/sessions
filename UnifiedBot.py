...
(ASK_MODE, ASK_COUNT, ASK_DBNAME, ASK_TABLENAME, 
 ADD_BOT_TOKEN, ASK_FILE_PATH, CONFIRM_ACTION, MANAGE_FILES) = range(8)
...
# إعدادات إدارة الملفات
SUPPORTED_FILE_EXTENSIONS = ['.txt', '.session', '.sessions']
DEFAULT_FILES_DIRECTORY = './session_files/'
...
def get_session_files():
    """الحصول على قائمة ملفات الجلسات المتاحة"""
    files = []
    ...
    # البحث في مجلد الملفات المخصص إذا كان موجوداً
    if os.path.exists(DEFAULT_FILES_DIRECTORY):
        for ext in SUPPORTED_FILE_EXTENSIONS:
            files.extend(glob.glob(f"{DEFAULT_FILES_DIRECTORY}*{ext}"))
...
def start(update: Update, context: CallbackContext):
    ...
    if CHECKER_AVAILABLE:
        keyboard.extend([
            ...
            [InlineKeyboardButton("🗂️ إدارة الملفات", callback_data="manage_files")]
        ])
...
def button_handler(update: Update, context: CallbackContext):
    ...
    if query.data.startswith("mode_"):
        ...
        return handle_file_management(query, context)
...
def handle_action_buttons(query, context):
    ...
    if action == "add_bot":
        ...
        return handle_file_action_buttons(query, context)
...
# ----------------------
# دوال إدارة الملفات الإضافية
# ----------------------

def handle_file_management(query, context):
    """معالج إدارة الملفات المحفوظة"""
    if not CHECKER_AVAILABLE:
        query.edit_message_text("❌ وحدة إدارة الملفات غير متاحة.")
        return ConversationHandler.END
    ...
    # عرض قائمة الملفات (أول 8 ملفات)
    files_text = ""
    keyboard = []
    
    for i, file_path in enumerate(session_files[:8]):
        file_name = os.path.basename(file_path)
        file_info = get_file_info(file_path)
        
        if file_info["exists"]:
            files_text += f"{i+1}. **{file_name}**\n"
            files_text += f"   📊 {file_info['lines']} جلسة | {file_info['size']} | {file_info['modified']}\n\n"
            
            # إضافة أزرار للملف
            keyboard.append([
                InlineKeyboardButton(f"🔍 فحص {file_name[:15]}...", callback_data=f"file_check_{i}"),
                InlineKeyboardButton(f"🗑️ حذف", callback_data=f"file_delete_{i}")
            ])
    
    # حفظ قائمة الملفات في context للاستخدام لاحقاً
    context.user_data["session_files"] = session_files[:8]
    
    keyboard.extend([
        [InlineKeyboardButton("🔄 تحديث القائمة", callback_data="manage_files")],
        [InlineKeyboardButton("🔙 رجوع", callback_data="back_to_main")]
    ])
...
def handle_file_action_buttons(query, context):
    """معالج أزرار إجراءات الملفات"""
    action_data = query.data
    
    if action_data.startswith("file_check_"):
        # فحص ملف
        file_index = int(action_data.replace("file_check_", ""))
        session_files = context.user_data.get('session_files', [])
        
        if 0 <= file_index < len(session_files):
            file_path = session_files[file_index]
            file_name = os.path.basename(file_path)
            ...
            query.edit_message_text(
                ...
                f"📁 الملف: `{file_name}`\n"
                f"📍 المسار: `{file_path}`\n\n"
                f"⏳ جاري بدء عملية الفحص...",
                parse_mode="Markdown"
            )
            
            # بدء فحص الملف
            def cleanup_file_check():
                checker.check_file_sessions(file_path, query.message.chat_id, context)
            
            thread = threading.Thread(target=cleanup_file_check, daemon=True)
            thread.start()
            
            checker.log_activity("UNIFIED_FILE_CHECK", f"بدء فحص ملف: {file_path}")
            return ConversationHandler.END
        else:
            query.edit_message_text("❌ ملف غير صالح.")
            return ConversationHandler.END
    
    elif action_data.startswith("file_delete_"):
        # حذف ملف
        file_index = int(action_data.replace("file_delete_", ""))
        session_files = context.user_data.get('session_files', [])
        ...
        if 0 <= file_index < len(session_files):
            ...
            keyboard = [
                [
                    InlineKeyboardButton("✅ نعم، احذف", callback_data=f"confirm_delete_{file_index}"),
                    InlineKeyboardButton("❌ إلغاء", callback_data="manage_files")
                ]
            ]
    ...
    elif action_data.startswith("confirm_delete_"):
        # تأكيد حذف الملف
        file_index = int(action_data.replace("confirm_delete_", ""))
        session_files = context.user_data.get('session_files', [])
        ...
        if 0 <= file_index < len(session_files):
            ...
            if delete_file_safe(file_path):
                query.edit_message_text(
                    f"✅ **تم الحذف بنجاح**\n\n"
                    f"تم حذف الملف: `{file_name}`",
                    parse_mode="Markdown",
                    reply_markup=InlineKeyboardMarkup([
                        [InlineKeyboardButton("🔙 رجوع لإدارة الملفات", callback_data="manage_files")]
                    ])
                )
                checker.log_activity("FILE_DELETED", f"تم حذف الملف: {file_name}")
            else:
                query.edit_message_text(
                    f"❌ **فشل في الحذف**\n\n"
                    f"تعذر حذف الملف: `{file_name}`\n"
                    f"تأكد من الصلاحيات أو أن الملف غير مستخدم.",
                    parse_mode="Markdown",
                    reply_markup=InlineKeyboardMarkup([
                        [InlineKeyboardButton("🔙 رجوع لإدارة الملفات", callback_data="manage_files")]
                    ])
